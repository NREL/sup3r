...
def test_fwd_pass_writing():
    """Test forward pass h5 writing."""
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        out_pattern = os.path.join(td, 'fwp_{file_id}.h5')
        cache_pattern = os.path.join(td, 'cache')
        handler = ForwardPassStrategy(
            input_files, model_args=out_dir,
            s_enhance=3, t_enhance=4,
        ...
        with ResourceX(handler.out_files[0]) as f:
            ws = f['windspeed_100m']
            wd = f['winddirection_100m']
            new_shape = (3 * shape[0], 3 * shape[1], 4 * len(input_files))
            ws = np.transpose(ws, (1, 0)).reshape(new_shape)
            wd = np.transpose(wd, (1, 0)).reshape(new_shape)
            data = np.concatenate([ws[..., np.newaxis], wd[..., np.newaxis]],
                                  axis=-1)
        ...
        handlerNC = DataHandlerNC(input_files, FEATURES,
                                  ...
                                  overwrite_cache=True,
                                  val_split=0.0)
        gen_dat = model.generate(np.expand_dims(handlerNC.data, axis=0))[0]

        gen_dat, _, _ = OutputHandlerH5._prep_output(gen_dat,
                                                     FEATURES[:2],
                                                     handlerNC.lat_lon,
                                                     handlerNC.time_index)
        assert np.allclose(data, gen_dat, rtol=0.01)
