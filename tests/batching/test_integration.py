...
from sup3r.containers.batchers import SplitBatchQueue
...
def test_train_spatial(log=True, full_shape=(20, 20), sample_shape=(10, 10, 1),
                       n_epoch=5):
    ...
    handler = DataHandlerH5(FP_WTK, FEATURES, target=TARGET_COORD,
                            ...
                            worker_kwargs={'max_workers': 1}, val_split=0.0)

    sampler = Sampler(handler, sample_shape)
    means = {FEATURES[i]: handler.data[..., i].mean()
             for i in range(len(FEATURES))}
    stds = {FEATURES[i]: handler.data[..., i].std()
            for i in range(len(FEATURES))}
    batch_handler = SplitBatchQueue([sampler], val_split=0.1,
                                    batch_size=2, s_enhance=2, t_enhance=1,
                                    n_batches=2, means=means, stds=stds)
...
def test_train_st(log=True, full_shape=(20, 20), sample_shape=(12, 12, 16),
                  n_epoch=5):
    ...
    stds = {FEATURES[i]: handler.data[..., i].std()
            for i in range(len(FEATURES))}
    batch_handler = SplitBatchQueue([sampler], val_split=0.1,
                                    batch_size=2, s_enhance=3, t_enhance=4,
                                    n_batches=2, means=means, stds=stds)
