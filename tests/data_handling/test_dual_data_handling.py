...
def test_regrid_caching(
    log=True, full_shape=(20, 20), sample_shape=(10, 10, 1)
):
...
def test_st_dual_batch_handler(
    ...
    for batch in batch_handler:
        for i, index in enumerate(batch_handler.current_batch_indices):
            ...
            assert np.array_equal(
                batch.high_res[i],
                dual_handler.hr_data[hr_index],
            )
            assert np.array_equal(
                batch.low_res[i],
                dual_handler.lr_data[lr_index],
            )


def test_spatial_dual_batch_handler(
    log=False, full_shape=(20, 20), sample_shape=(10, 10, 1), plot=True
):
    ...
    dual_handler = DualDataHandler(
        hr_handler, lr_handler, s_enhance=2, t_enhance=1, val_split=0.0
    )

    batch_handler = SpatialDualBatchHandler(
        [dual_handler], batch_size=2, s_enhance=2, t_enhance=1, n_batches=10
    )
