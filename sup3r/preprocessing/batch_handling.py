        ...
        for i, h in enumerate(self.data_handlers):
            if h.val_data is not None:
                for _ in range(h.val_data.shape[2]):
                    ...
                    val_indices.append(
                        {'handler_index': i, 'tuple_index': tuple_index}
                    )
        return val_indices
...
class BatchHandler:
    ...
    def __init__(
        ...
        logger.info(
            f'Initializing BatchHandler with smoothing={smoothing}. '
    ...
    def stats_workers(self):
        ...
        stats_workers = estimate_max_workers(
            self._stats_workers, proc_mem, len(self.data_handlers)
        )
    ...
    def load_workers(self):
        ...
        max_workers = estimate_max_workers(
            self._load_workers, proc_mem, len(self.data_handlers)
        )
        return max_workers
    ...
    def norm_workers(self):
        ...
        norm_workers = estimate_max_workers(
            self._norm_workers, proc_mem, len(self.training_features)
        )
        return norm_workers
    ...
    def parallel_normalization(self):
        ...
        if max_workers == 1:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as exe:
                ...
                logger.info(
                    f'Started normalizing {len(self.data_handlers)} '
                ...
                for i, _ in enumerate(as_completed(futures)):
                    try:
                        ...
                        msg = (
                            'Error normalizing data handler number '
                            f'{futures[future]}'
                        )
                        logger.exception(msg)
                        raise RuntimeError(msg) from e
                    logger.debug(
                        f'{i+1} out of {len(futures)} data handlers'
                        ' normalized.'
                    )
    ...
    def parallel_load(self):
        ...
        if max_workers == 1:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as exe:
                ...
                logger.info(
                    f'Started loading all {len(self.data_handlers)} '
                    f'data handlers in {dt.now() - now}.'
                )
                ...
                for i, future in enumerate(as_completed(futures)):
                    try:
                        ...
                        msg = (
                            'Error loading data handler number '
                            f'{futures[future]}'
                        )
                        logger.exception(msg)
                        raise RuntimeError(msg) from e
    ...
    def parallel_stats(self):
        ...
        parallel."""
        logger.info(
            f'Calculating stats for {len(self.training_features)} ' 'features.'
        )
    ...
    def check_cached_stats(self):
        ...
        """
        stdevs_check = (
            self.stdevs_file is not None and not self.overwrite_stats
        )
        ...
        if stdevs_check and means_check:
            ...
            msg = (
                'The training features and cached statistics are '
    ...
    def get_stdevs_for_feature(self, feature, max_workers=None):
        ...
        if max_workers == 1:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as exe:
                ...
                for didx, _ in enumerate(self.data_handlers):
                    future = exe.submit(
                        self.get_handler_variance, idx, didx, self.means[idx]
                    )
                    futures[future] = didx

                logger.info(
                    'Started calculating stdevs for '
                    f'{len(self.data_handlers)} data_handlers in '
                    f'{dt.now() - now}.'
                )
    ...
    def normalize(self, means=None, stds=None):
        ...
        logger.info(
            'Finished normalizing data in all data handlers in '
            f'{dt.now() - now}.'
        )
    ...
    def __next__(self):
        ...
        for i in range(self.batch_size):
            ...
            if high_res is None:
                ...
                msg = (
                    'SpatialBatchHandlerCC can only use n_temporal==1 '
    ...
    def __init__(self, *args, **kwargs):
        ...
        **kwargs : dict
        ...
        self.spatial_bins = np.array_split(
            np.arange(0, bin_range), self.val_data.N_SPACE_BINS
        )
