...
from sup3r.qa.feature_plots import Sup3rVisualQa
...
def from_config(ctx, config_file, verbose):
    ...
    name = 'sup3r_visual_qa_{}'.format(os.path.basename(status_dir))
    ctx.obj['NAME'] = name
    config['job_name'] = name
    config['status_dir'] = status_dir

    cmd = Sup3rVisualQa.get_node_cmd(config)
...
def kickoff_local_job(ctx, cmd):
    ...
    cmd : str
        Command to be submitted in shell script. Example:
            'python -m sup3r.cli visual-qa -c <config_file>'
    """
...
def kickoff_slurm_job(ctx, cmd, alloc='sup3r', memory=None, walltime=4,
    ...
    """Run sup3r visual QA on HPC via SLURM job submission.
    ...
    if status == 'successful':
        ...
        logger.info('Running sup3r visual QA on SLURM with node name "{}".'
                    .format(name))
        ...
        if out:
            msg = ('Kicked off sup3r visual QA job "{}" '
                   '(SLURM jobid #{}).'
                   .format(name, out))
