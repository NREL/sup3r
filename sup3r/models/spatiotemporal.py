...
class SpatioTemporalGan(BaseModel):
    ...
    def update_adversarial_weights(self, history, adaptive_update_fraction,
        ...
        train_disc_t : bool
            ...
            previous epoch
        weight_update_int : int Number of previous epochs to average
        loss_details[comparison_key] over before checking for update
        conditions. e.g. If weight_update_int = 2 then the update condition
        will depend on the average of loss_details[comparison_key] over the
        previous 2 epochs.
    ...
    def train(self, batch_handler, n_epoch,
        ...
        for epoch in epochs:
            ...
            out = self.update_adversarial_weights(
                ...
                train_disc_t, weight_update_int=weight_update_int)
...
class SpatioTemporalGanKLD(SpatioTemporalGan):
    """SpatioTemporalGan sub class using mse + kl divergence instead of just
    mse"""
...
class SpatioTemporalGanMMD(SpatioTemporalGan):
    """SpatioTemporalGan sub class using mse + max mean discrepancy for content
