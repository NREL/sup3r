...
from sup3r.models.abstract import AbstractInterface, AbstractSingleModel, AbstractMultiModel
from sup3r.utilities import VERSION_RECORD
from sup3r.models.multi_step import MultiStepGan

logger = logging.getLogger(__name__)


class MultiSup3rCondMom(AbstractInterface, AbstractMultiModel):
    """Basic Sup3r conditional moments model."""

    def __init__(self, gen_layers_list, gen_s_en_list, gen_t_en_list,
                 optimizer=None, learning_rate=1e-4, num_par_list=None,
                 history=None, meta_list=None, means=None, stdevs=None,
                 default_device=None, name_list=None):
        """
        Parameters
        ----------
        gen_layers : list | str
        ...
        self.name_list = name_list if name_list is not None else [self.__class__.__name__]*len(gen_s_en_list)
        ...
        self._gen_list = [self.load_network(gen_layers, f'generator_{igen}') for igen, gen_layers in enumerate(gen_layers_list)]
        self._s_en_list = gen_s_en_list
        self._t_en_list = gen_t_en_list
        self._meta_list = meta_list if meta_list is not None else [{}]*len(gen_s_en_list)
        self._num_par_list = num_par_list if num_par_list is not None else [0] * len(gen_s_en_list)
    ...
    def meta_list(self):
        ...
        for im,_meta in enumerate(self._meta_list):
            if 'class' not in self._meta_list[im]:
                self._meta_list[im]['class'] = self.__class__.__name__
    ...
    def model_params_list(self):
        ...
        for im, weights in enumerate(self.weights_list):
            num_par = int(np.sum(
                          [np.prod(v.get_shape().as_list())
                           for v in weights]))

            model_params = {'name': self.name_list[im],
                            'num_par': num_par,
                            'version_record': self.version_record,
                            'optimizer': config_optm_g,
                            'means': means,
                            'stdevs': stdevs,
                            'meta': self.meta_list[im],
                            }
    ...
    def save_params(self, out_dir):
        ...
        for im, (s_en, t_en) in enumerate(zip(self._s_en_list, self._t_en_list)):
            fp_params = os.path.join(out_dir, 'model_params_{s_en}_{t_en}.json')
            with open(fp_params, 'w') as f:
                params = self.model_params_list[im]
    ...
    def save(self, out_dir):
        ...
        for im, (s_en, t_en) in enumerate(zip(self._s_en, self._t_en)):
            fp_gen = os.path.join(out_dir, 'model_gen_{s_en}_{t_en}.pkl')
            self.generator_list[im].save(fp_gen)
    ...
    def load(cls, model_dir, verbose=True):
        ...
        fp_gen_list = []
        for im, (s_en, t_en) in enumerate(zip(self._s_en, self._t_en)):
            fp_gen_list.append(os.path.join(model_dir, 'model_gen_{s_en}_{t_en}.pkl'))
        params_list = cls.load_saved_params(model_dir, verbose=verbose)

        return cls(fp_gen_list, params_list)
    ...
    def generate(self, low_res, norm_in=True, un_norm_out=True,
        ...
        for generator in self.generator_list:
            output = generator.layers[0](output)
            for i, layer in enumerate(generator.layers[1:]):
                try:
                    output = layer(output)
    ...
    def weights_list(self):
        ...
        generator network
        """
        return [generator.weights for generator in self.generator_list]
