...
class AbstractMultiModel(ABC):
    ...
    def __init__(self):
        ...
        self._gen_list = None
        self._n_gen = None
        self._s_en_list = None
        self._t_en_list = None
        self._means = None
        self._stdevs = None
    ...
    def load_network(self, model, name):
        ...
        if isinstance(model, str) and model.endswith('.json'):
            model = safe_json_load(model)
            #breakpoint()
            #self._meta_list[ind][f'config_{name}'] = model
            if 'hidden_layers' in model:
                model = model['hidden_layers']
            elif ('meta' in model
                  and f'config_{name}' in model['meta']
    ...
    def load_saved_params(out_dir, verbose=True):
        ...
        params_list = []
        for (s_en, t_en) in zip(self.s_en_list, self.t_en_list):
