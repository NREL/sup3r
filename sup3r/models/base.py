...
class BaseModel(ABC):
    ...
    def __init__(self, optimizer=None, content_loss_metric=None,
                 learning_rate=1e-4,
        ...
        if content_loss_metric is None:
            self.content_loss_metric = mean_squared_error
        else:
            self.content_loss_metric = content_loss_metric
    ...
    def update_content_loss_metric(self, loss_metric):
        """
        Update the method used to calculate content loss

        loss_metric : tensorflow.keras.metrics
            Method used to compute content loss. e.g. kl_divergence.
        """

        self.content_loss_metric = loss_metric

    @property
    def get_content_loss_metric(self):
        """Get name of content loss metric"""
        return self.content_loss_metric.__name__
    ...
    @staticmethod
    @tf.function
    def calc_loss_gen_content(hi_res_true, hi_res_gen, loss_metric=None):
        ...
        hi_res_gen : tf.Tensor
            ...
            generative model.
        loss_metric : tf.keras.metrics | None
            Metric used to calculate content loss. e.g. kl_divergence.
            None defaults to mean_squared_error.
        ...
        loss_gen_content = loss_metric(hi_res_true, hi_res_gen)
