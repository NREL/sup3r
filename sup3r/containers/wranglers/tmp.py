...
class Wrangler(AbstractWrangler, FeatureDeriver, ABC):
    ...
    def to_h5(self, out_file, data=None, lat_lon=None, features=None,
              chunks=None):
        """Save data to h5 file with appropriate lat/lon/time.
        ...
        features : list
            ...
            self.features will be used.
        chunks : dict
            Dictionary of chunks args for each feature to write
        """
        os.makedirs(os.path.dirname(out_file), exist_ok=True)
        data = data if data is not None else self.data
        lat_lon = lat_lon if lat_lon is not None else self.lat_lon
        ...
        if out_file is not None:
            ...
            with h5py.File(out_file, 'w') as f:
                ...
                for fidx, feat in enumerate(self.features):
                    f.create_dataset(feat, data=data[..., fidx],
                                     chunks=chunks[feat])
    ...
    def try_load(self):
        ...
        return self._should_load_cache(
            self.cache_pattern, self.cache_files, self.overwrite_cache
        )
    ...
    def cache_files(self):
        ...
        if self.cache_pattern is not None:
            return [
                self.cache_pattern.format(feature=f) for f in self.features
            ]
        return None
    ...
    def _load_single_cached_feature(
        ...
        logger.info(
            f'Loading {features[idx]} from {fp}. Current memory '
    ...
    def _load_cached_data(self, data, cache_files, features, max_workers=None):
        ...
        if max_workers == 1:
            ...
            self.parallel_load(
                data, cache_files, features, max_workers=max_workers
            )

    @staticmethod
    def check_cached_features(
        features, cache_files=None, overwrite_cache=False, load_cached=False
    ):
    ...
    def raw_time_index(self):
        ...
        if self._raw_time_index is None:
            self._raw_time_index = self.get_time_index(
                self.file_paths, **self.res_kwargs
            )
