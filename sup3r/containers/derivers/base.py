...
from sup3r.containers.derivers.factory import RegistryBase
...
class Deriver(Container):
    ...
    def update_data(self):
        ...
        after the transform then the calls to __getitem__ will run derivations
        for features found in the feature registry."""
        if self.transform is not None:
            self.container.data = self.transform(self.container)
        self.data = da.stack([self[feat] for feat in self.features], axis=-1)

    def check_for_compute(self, feature):
    ...
    def __getitem__(self, keys):
        ...
        if isinstance(key, str):
            ...
            compute = self.check_for_compute(key)
