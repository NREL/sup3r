    ...
    if args.indMoment == 1:
        if args.subfilter:
            from sup3r.preprocessing.batch_handling import (
                BatchHandlerMom1SF as BatchHandler)
        else:
            from sup3r.preprocessing.batch_handling import (
                BatchHandler as BatchHandler)
    elif args.indMoment == 2:
        if args.mom1_file is None:
            sys.exit("ERROR: Need first moment file to train second moment")
        if args.subfilter:
            from sup3r.preprocessing.batch_handling import (
                BatchHandlerMom2SF as BatchHandler)
        else:
            from sup3r.preprocessing.batch_handling import (
                BatchHandlerMom2 as BatchHandler)
    else:
        sys.exit("ERROR: Moment must be 1 or 2")

if not spatio_temp:
    if args.indMoment == 1:
        if args.subfilter:
            from sup3r.preprocessing.batch_handling import (
                SpatialBatchHandlerMom1SF as BatchHandler)
        else:
            from sup3r.preprocessing.batch_handling import (
                SpatialBatchHandler as BatchHandler)
    elif args.indMoment == 2:
        if args.mom1_file is None:
            sys.exit("ERROR: Need first moment file to train second moment")
        if args.subfilter:
            from sup3r.preprocessing.batch_handling import (
                SpatialBatchHandlerMom2SF as BatchHandler)
        else:
            from sup3r.preprocessing.batch_handling import (
                SpatialBatchHandlerMom2 as BatchHandler)
    else:
        sys.exit("ERROR: Moment must be 1 or 2")
...
localFolder_file = '%s_mom%d%s_%d_spad%d_tpad%d' % (coord_str,
                                                    args.indMoment,
                                                    sf_str,
                                                    args.indModel,
                                                    spad,
                                                    tpad)
output_root = '/projects/nationalwind/conditionalMoment/output'
model_otpt = os.path.join(output_root, localFolder_file)
os.makedirs(output_root, exist_ok=True)
os.makedirs(model_otpt, exist_ok=True)


logger = logging.getLogger(__name__)
config_root = '/projects/nationalwind/conditionalMoment/real_configs'
if spatio_temp:
    json_file = 'gen_2x_12x_14f%s_%d.json' % (mom_str, args.indModel)
else:
    json_file = 'gen_2x_1x_14f%s_%d.json' % (mom_str, args.indModel)
fp_gen = os.path.join(config_root, json_file)

saved_model = None  # train model from scratch
out_dir = model_otpt + '/cond_mom_e{epoch}/'
...
if __name__ == '__main__':
    ...
    cache_root = '/scratch/mhassana/wtk_cache'
    os.makedirs(cache_root, exist_ok=True)
    os.makedirs(os.path.join(cache_root,
                             localFolder_file),
                exist_ok=True)
    os.makedirs(os.path.join(cache_root,
                             localFolder_file,
    ...
    for height in [10, 40, 80, 100, 120, 160, 200]:
        ...
        dataFile = os.path.join('/datasets/WIND/conus/v1.0.0/',
                                f'{year}/wtk_conus_{year}_{height}m.h5')
        wtk_files.append(dataFile)
